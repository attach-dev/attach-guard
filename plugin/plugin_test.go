...
func TestBootstrapDoesNotRequireSocketToken(t *testing.T) {
	...
	if lines[0] != "" {
		t.Fatalf("expected empty socket token, got %q", lines[0])
	}
	if lines[1] != "version" {
...
func testEnvWithoutSocket() []string {
	...
	for _, entry := range os.Environ() {
		if !strings.HasPrefix(entry, "SOCKET_API_TOKEN=") &&
			!strings.HasPrefix(entry, "CLAUDE_PLUGIN_OPTION_socket_api_token=") &&
			!strings.HasPrefix(entry, "CLAUDE_PLUGIN_OPTION_SOCKET_API_TOKEN=") {
